...
[Code]
var LicensePage:TInputQueryWizardPage;RolePage:TInputOptionWizardPage;
procedure InitializeWizard;begin LicensePage:=CreateInputQueryPage(wpWelcome,'Installation key','Enter the Mnahel''s Cafe POS installation key','This key is required to continue.');LicensePage.Add('Installation key:',False);RolePage:=CreateInputOptionPage(LicensePage.ID,'Is PC ka kirdar','Yeh PC server hai ya cashier counter?','SERVER PC par database aur background service install hoti hai. CASHIER PC network par server se connect karta hai.',True,False);RolePage.Add('SERVER PC (Admin) - database aur POS service yahan chalegi');RolePage.Add('CASHIER PC - network par maujood server se connect karega');RolePage.SelectedValueIndex:=0;end;
function IsServerRole:Boolean;begin Result:=RolePage.SelectedValueIndex=0;end;
function IsClientRole:Boolean;begin Result:=not IsServerRole;end;
function NextButtonClick(CurPageID:Integer):Boolean;begin Result:=True;if CurPageID=LicensePage.ID then if CompareText(Trim(LicensePage.Values[0]),'{#InstallKey}')<>0 then begin MsgBox('Invalid installation key.',mbError,MB_OK);Result:=False;end;end;
function PrepareToInstall(var NeedsRestart:Boolean):String;var ResultCode:Integer;begin Exec(ExpandConstant('{sys}\sc.exe'),'stop MnahelsCafePOS','',SW_HIDE,ewWaitUntilTerminated,ResultCode);Sleep(1500);Result:='';end;
procedure CurStepChanged(CurStep:TSetupStep);var ConfigPath:String;begin if CurStep=ssPostInstall then begin ConfigPath:=ExpandConstant('{commonappdata}\MnahelsCafePOS\connection.json');if IsServerRole then SaveStringToFile(ConfigPath,'{"mode":"server","serverUrl":"http://localhost:5055"}',False) else SaveStringToFile(ConfigPath,'{"mode":"client","serverUrl":""}',False);DelTree(ExpandConstant('{localappdata}\MnahelsCafePOS\WebView2-Cashier'),True,True,True);DelTree(ExpandConstant('{localappdata}\MnahelsCafePOS\WebView2-Admin'),True,True,True);end;end;