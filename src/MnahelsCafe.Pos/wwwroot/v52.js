...
const cash=value=>`Rs ${Number(value||0).toLocaleString('en-PK',{maximumFractionDigits:2})}`;
...
function toppingFor(item){const context=modifierContext(item),product=toppingProduct();if(!context?.size||!product)return null;const variant=(product.variants||[]).find(x=>String(x.name).trim().toLowerCase()===context.size.toLowerCase());return variant?{variantId:Number(variant.id),name:variant.name,price:Number(variant.price||0),context}:null}
function quoteTopping(item,topping){const subject=topping.context.kind==='deal'?`${item.name} pizza`:`${item.name}`;return{variantId:topping.variantId,price:topping.price,name:topping.name,notes:`For ${subject} (${topping.context.size})`}}
function toppingPrice(item){return item?.extraTopping?Number(item.extraToppingQuote?.price??toppingFor(item)?.price??0):0}
...
function hideStandaloneTopping(){const ids=new Set(entries().filter(({product})=>String(product.name||'').trim().toLowerCase()==='extra topping').map(({product})=>Number(product.id)));qa('#product-grid .product-card').forEach(card=>{if(ids.has(Number(card.dataset.id)))card.remove()})}
function checkboxFor(row,item,topping){
 ...
 if(item.extraTopping&&!item.extraToppingQuote)item.extraToppingQuote=quoteTopping(item,topping);
 const input=q('input',label),price=q('b',label);input.checked=!!item.extraTopping;price.textContent=`+${cash(item.extraToppingQuote?.price??topping.price)}`;label.classList.toggle('active',input.checked);label.title=`${topping.context.kind==='deal'?'Deal pizza':'Pizza'} ${topping.name} extra topping · ${cash(item.extraToppingQuote?.price??topping.price)}`;
 input.onchange=event=>{event.stopPropagation();const latest=event.currentTarget.checked?toppingFor(item):null;item.extraTopping=!!latest;item.extraToppingQuote=latest?quoteTopping(item,latest):null;window.renderCart?.()};label.onclick=event=>event.stopPropagation();
}
...
function expandedItems(items){const topping=toppingProduct(),toppingIds=new Set([...(topping?.variants||[]).map(x=>Number(x.id)),...(state?.cart||[]).map(x=>x.extraToppingQuote?.variantId).filter(Boolean)]);if((items||[]).some(x=>toppingIds.has(Number(x.variantId))))return items;const out=[];for(const line of items||[]){out.push(line);const item=(state?.cart||[]).find(x=>Number(x.variantId)===Number(line.variantId)),extra=item?.extraTopping?item.extraToppingQuote:null;if(item?.extraTopping&&!extra)throw Error('Extra topping dobara select karke price review karein.');if(extra)out.push({variantId:extra.variantId,quantity:Number(line.quantity||item.quantity||1),notes:extra.notes})}return out}
function priceLines(){return(state?.cart||[]).flatMap(item=>[item,...(item.extraTopping&&item.extraToppingQuote?[{...item.extraToppingQuote,quantity:item.quantity}]:[])])}
function hookApi(){const current=window.api;if(typeof current!=='function'||current===apiHook)return;const wrapped=async function(path,options={}){const method=String(options.method||'GET').toUpperCase();if((path==='/api/orders'||path==='/api/orders/book')&&method==='POST'&&options.body){const body=JSON.parse(options.body);body.items=expandedItems(body.items);body.discount=discountAmount();options={...options,body:JSON.stringify(body)}}return current(path,options)};wrapped.__v52=true;apiHook=wrapped;window.api=wrapped}
...
window.mnahelsV52={build:BUILD,uiRevision:REV,subtotal,discountAmount,toppingFor,modifierContext,expandedItems,priceLines,decorateCart};
})();
