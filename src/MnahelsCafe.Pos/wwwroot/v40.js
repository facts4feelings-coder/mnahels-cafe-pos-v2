...
function decorateImages(){
 qa('#product-grid .product-card').forEach(card=>{const found=productById(card.dataset.id);if(!found)return;const src=imageFor(found.product.name,found.category.name),media=q('.ma-food-media',card);if(media&&!media.style.backgroundImage){media.dataset.v40Image=src;media.style.backgroundImage=`url("${src}")`;card.dataset.v40Image=src}});
 qa('#cart-items .v37-cart-line').forEach((row,index)=>{const item=state?.cart?.[index];if(!item)return;const src=imageFor(item.name),img=q('.v37-cart-thumb',row);if(img&&!img.getAttribute('src')){img.src=src;img.dataset.v40Image=src}});
 qa('#admin-orders .v36-food').forEach(tile=>{const src=imageFor(q('strong',tile)?.textContent||tile.title),img=q('img',tile);if(img&&!img.getAttribute('src')){img.src=src;img.dataset.v40Image=src}});
}
function hookRenderers(){
 const products=window.renderProducts;if(typeof products==='function'&&!renderProductsHooked&&!products.__v40){const wrapped=function(){const out=products.apply(this,arguments);requestAnimationFrame(decorateImages);return out};wrapped.__v40=true;renderProductsHooked=wrapped;window.renderProducts=wrapped}
 const cart=window.renderCart;if(typeof cart==='function'&&!renderCartHooked&&!cart.__v40){const wrapped=function(){const out=cart.apply(this,arguments);requestAnimationFrame(decorateImages);return out};wrapped.__v40=true;renderCartHooked=wrapped;window.renderCart=wrapped}
 decorateImages();
}
...
function hookComplete(){const current=window.showOrderComplete;if(typeof current!=='function'||completeHooked||current.__v40)return;const wrapped=function(order){const out=current.apply(this,arguments);showSuccess(order);return out};wrapped.__v40=true;completeHooked=wrapped;window.showOrderComplete=wrapped}

function setNavLabel(screen,label){const button=q(`.sidebar nav [data-screen="${screen}"]`);if(!button)return;let node=q('.ma-nav-label',button);if(!node){node=document.createElement('span');node.className='ma-nav-label';button.appendChild(node)}if(node.textContent!==label)node.textContent=label;if(button.title!==label)button.title=label}
function arrangeNavigation(){const nav=q('.sidebar nav');if(!nav)return;setNavLabel('menu-admin','Menu Manager');setNavLabel('pos','Our Menu');setNavLabel('service','Service Hub');const desired=['admin','pos','shift','sales','customers','service','menu-admin','settings','orders'],nodes=desired.map(x=>q(`[data-screen="${x}"]`,nav)).filter(Boolean),current=[...nav.children].filter(x=>nodes.includes(x));if(nodes.some((x,i)=>current[i]!==x))nodes.forEach(x=>nav.appendChild(x));if(state?.currentScreen==='pos'){const title=q('#page-title'),kicker=q('#page-kicker');if(title&&title.textContent!=='Our Menu')title.textContent='Our Menu';if(kicker&&kicker.textContent!=='FRONT COUNTER')kicker.textContent='FRONT COUNTER'}if(!nav.dataset.v40Observed){nav.dataset.v40Observed='1';new MutationObserver(()=>queueMicrotask(arrangeNavigation)).observe(nav,{childList:true})}}
...
function hookSales(){const current=window.loadSales;if(typeof current!=='function'||salesHooked||current.__v40)return;const wrapped=async function(){const out=await current.apply(this,arguments);cleanSales();return out};wrapped.__v40=true;salesHooked=wrapped;window.loadSales=wrapped;cleanSales()}

function observeDynamic(){if(document.documentElement.dataset.v40Observed)return;document.documentElement.dataset.v40Observed='1';const observer=new MutationObserver(mutations=>{let images=false,context=false,sales=false;for(const mutation of mutations){const node=mutation.target;if(node.closest?.('#product-grid,#cart-items,#admin-orders'))images=true;if(node.id==='v38-order-context'||node.closest?.('#v38-order-context'))context=true;if(node.id==='one-metrics'||node.closest?.('#one-metrics,#one-chart'))sales=true;for(const added of mutation.addedNodes){if(added.nodeType!==1)continue;if(added.id==='v38-order-context'||added.querySelector?.('#v38-order-context'))context=true;if(added.matches?.('.product-card,.v37-cart-line,.v36-food')||added.querySelector?.('.product-card,.v37-cart-line,.v36-food'))images=true}}if(images)requestAnimationFrame(decorateImages);if(context)requestAnimationFrame(decorateContext);if(sales)requestAnimationFrame(cleanSales)});qa('#product-grid,.cart-panel,#admin-orders,#screen-sales').forEach(root=>observer.observe(root,{childList:true,subtree:true}))}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(boot,900));else setTimeout(boot,900);setTimeout(()=>{boot();refreshDashboardFilter()},1600);setTimeout(boot,3100);document.addEventListener('click',event=>{if(event.target.closest?.('.nav-item,#login-form button'))requestAnimationFrame(()=>{arrangeNavigation();ensureWipeUi()})});document.addEventListener('cafe:catalog-updated',()=>{arrangeNavigation();ensureWipeUi()});
