...
static class ShiftFeatures
{
    ...
    static async Task<IResult> Pdf(long id,PosDb db){var s=await db.Shifts.AsNoTracking().FirstOrDefaultAsync(x=>x.Id==id);if(s is null)return Results.NotFound();if(s.Status!="Closed")return Results.Conflict(new{message="Z-report ke liye pehle shift close karein."});var b=ZPdf.Create(await Summary(db,s),await ShiftAuditReport.Read(db,s));return Results.File(b,"application/pdf",$"Mnahels-{s.ShiftNumber}-Z-Report.pdf");}
    ...
    static class ZPdf
    {
        public static byte[] Create(ShiftSummary s, ShiftAuditDocument audit)
        {
            ...
            Box(45,708,505,46);
            Text(55,744,11,$"REPORT  {s.ShiftNumber}",true);
            Text(330,744,9,$"COUNTER  {s.CounterName}",true);
            Text(55,730,10,$"Opened  {s.OpenedAt.ToOffset(TimeSpan.FromHours(5)):dd MMM yyyy}");
            Text(55,716,10,$"{s.OpenedAt.ToOffset(TimeSpan.FromHours(5)):hh:mm:ss tt}");
            Text(330,730,10,$"Closed  {s.ClosedAt?.ToOffset(TimeSpan.FromHours(5)):dd MMM yyyy}");
            ...
            Text(58,428,8,$"Paid {s.PaidOrders}   |   Outstanding {s.OutstandingOrders}   |   Cancelled {s.CancelledOrders}   |   Edited {s.EditedOrders}");

            Fill(45,380,245,24,"0.91");Box(45,380,245,24);Text(55,388,9,"OPERATOR BREAKDOWN",true);
            Fill(305,380,245,24,"0.91");Box(305,380,245,24);Text(315,388,9,"ORDER ACTIVITY",true);
            ...
            return ShiftAuditPdf.Create(c.ToString(),audit);
        }
...
sealed class ShiftSummary{public bool Open{get;set;}public long Id{get;set;}public string ShiftNumber{get;set;}="";public string CounterName{get;set;}="";public string CashierName{get;set;}="";public string Status{get;set;}="";public DateTimeOffset OpenedAt{get;set;}public string OpenedByName{get;set;}="";public string OpenedByRole{get;set;}="";public DateTimeOffset? ClosedAt{get;set;}public string? ClosedByName{get;set;}public string? ClosedByRole{get;set;}public decimal OpeningCash{get;set;}public decimal GrossSales{get;set;}public decimal Discounts{get;set;}public decimal TotalPaidSales{get;set;}public int PaidOrders{get;set;}public decimal CashSales{get;set;}public decimal CardSales{get;set;}public decimal OnlineSales{get;set;}public int OutstandingOrders{get;set;}public decimal OutstandingAmount{get;set;}public int CancelledOrders{get;set;}public int EditedOrders{get;set;}public List<OrderLogEvent> OrderEvents{get;set;}=[];public decimal CashAdded{get;set;}public decimal CashOut{get;set;}public decimal ExpectedCash{get;set;}public decimal? ActualCash{get;set;}public decimal? Difference{get;set;}public string? DifferenceReason{get;set;}public string? AdminOverrideReason{get;set;}public List<ShiftOperator> Operators{get;set;}=[];public List<ShiftPayment> RecentPayments{get;set;}=[];public List<ShiftMove> Movements{get;set;}=[];public List<ShiftTop> TopItems{get;set;}=[];}
