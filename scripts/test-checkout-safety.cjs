...
function fixture(print=false){
 ...
 context.fetch=async(input,options={})=>{const path=typeof input==='string'?input:input.url;requests.push({path,options});if(path.endsWith('/revision'))return Response.json(remote);if(path.endsWith('/menu'))return Response.json([{id:1,name:'Pizza',products:[{id:1,isActive:true,isAvailable:true,variants:[{id:1,price:100}]}]}]);return Response.json({saved:true})};context.window=context;
